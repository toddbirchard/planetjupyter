...
$.ajax({url:api_url+"?key=5b439c179073fae7b9928e83dc64e969bd01b9562d693&q="+$(this).text(),contentType:"application/json",dataType:'json',success:function(result){var link_image=verifyImage(result.image);var verify_title=verifyTitle(result);console.log(result);$(element).after('<form action="{{ url_for(`notebookResult`) }}" method="post" class="recentnotebook">< <input type="submit" class="link-preview"><h4>'+verify_title+'</h4><div class="link-info">'+link_image+'<p>'+result.description+'</p></div></input></form>');$(element).remove();$("a").click(function(event){event.preventDefault();target=target.replace("https://github.com/","https://raw.githubusercontent.com/")
